...
class MockWeatherGenerator:
    ...
    def __init__(self, base_timestamp=None):
        ...
        self.weather_conditions = {
            'clear': {'id': 800, 'main': 'Clear', 'description': 'clear sky', 'icon_day': '01d', 'icon_night': '01n'},
            'few_clouds': {'id': 801, 'main': 'Clouds', 'description': 'few clouds', 'icon_day': '02d', 'icon_night': '02n'},
            'scattered_clouds': {'id': 802, 'main': 'Clouds', 'description': 'scattered clouds', 'icon_day': '03d', 'icon_night': '03n'},
            'overcast': {'id': 804, 'main': 'Clouds', 'description': 'overcast clouds', 'icon_day': '04d', 'icon_night': '04n'},
            'light_rain': {'id': 500, 'main': 'Rain', 'description': 'light rain', 'icon_day': '10d', 'icon_night': '10n'},
            'moderate_rain': {'id': 501, 'main': 'Rain', 'description': 'moderate rain', 'icon_day': '10d', 'icon_night': '10n'},
            'heavy_rain': {'id': 502, 'main': 'Rain', 'description': 'heavy intensity rain', 'icon_day': '10d', 'icon_night': '10n'},
            'light_snow': {'id': 600, 'main': 'Snow', 'description': 'light snow', 'icon_day': '13d', 'icon_night': '13n'},
            'moderate_snow': {'id': 601, 'main': 'Snow', 'description': 'snow', 'icon_day': '13d', 'icon_night': '13n'},
            'heavy_snow': {'id': 602, 'main': 'Snow', 'description': 'heavy snow', 'icon_day': '13d', 'icon_night': '13n'},
            'mist': {'id': 701, 'main': 'Mist', 'description': 'mist', 'icon_day': '50d', 'icon_night': '50n'},
            'fog': {'id': 741, 'main': 'Fog', 'description': 'fog', 'icon_day': '50d', 'icon_night': '50n'},
            'thunderstorm': {'id': 200, 'main': 'Thunderstorm', 'description': 'thunderstorm with light rain', 'icon_day': '11d', 'icon_night': '11n'}
        }

        # Season-appropriate weather probabilities
        self.season_weather_probs = {
            'winter': {
                'clear': 0.15, 'few_clouds': 0.1, 'scattered_clouds': 0.1, 'overcast': 0.2,
                'light_snow': 0.15, 'moderate_snow': 0.1, 'heavy_snow': 0.05,
                'mist': 0.05, 'fog': 0.1, 'light_rain': 0.0, 'moderate_rain': 0.0, 'heavy_rain': 0.0, 'thunderstorm': 0.0
            },
            'spring': {
                'clear': 0.2, 'few_clouds': 0.15, 'scattered_clouds': 0.15, 'overcast': 0.15,
                'light_rain': 0.15, 'moderate_rain': 0.1, 'heavy_rain': 0.02,
                'light_snow': 0.03, 'moderate_snow': 0.02, 'heavy_snow': 0.01,
                'mist': 0.02, 'fog': 0.0, 'thunderstorm': 0.0
            },
            'summer': {
                'clear': 0.35, 'few_clouds': 0.25, 'scattered_clouds': 0.15, 'overcast': 0.05,
                'light_rain': 0.1, 'moderate_rain': 0.05, 'heavy_rain': 0.02, 'thunderstorm': 0.03,
                'light_snow': 0.0, 'moderate_snow': 0.0, 'heavy_snow': 0.0, 'mist': 0.0, 'fog': 0.0
            },
            'fall': {
                'clear': 0.15, 'few_clouds': 0.1, 'scattered_clouds': 0.15, 'overcast': 0.25,
                'light_rain': 0.15, 'moderate_rain': 0.1, 'heavy_rain': 0.05,
                'light_snow': 0.02, 'moderate_snow': 0.01, 'heavy_snow': 0.0,
                'mist': 0.02, 'fog': 0.0, 'thunderstorm': 0.0
            }
        }
    ...
    def generate_weather_condition(self, season, is_day=True):
        ...
        condition = {
            ...
            'icon': base_condition['icon_day'] if is_day else base_condition['icon_night']
        }
    ...
    def generate_forecast_item(self, timestamp, season, weather_condition=None, base_temp=None):
        """Generate a single forecast item"""
        # Calculate sunrise/sunset (approximations)
        dt = datetime.fromtimestamp(timestamp)
        sunrise_hour = 6 + (2 if season in ['winter', 'fall'] else -1 if season == 'summer' else 0)
        sunset_hour = 18 + (-2 if season in ['winter', 'fall'] else 2 if season == 'summer' else 0)

        sunrise_ts = int(datetime(dt.year, dt.month, dt.day, sunrise_hour, 30).timestamp())
        ...
        if any(word in weather_condition['main'].lower() for word in ['rain', 'snow', 'thunderstorm']):
    ...
    def generate_mock_forecast(self, scenario='winter_storm', hours_ahead=40):
        ...
        for i in range(hours_ahead // 3):
            ...
            if scenario == 'winter_storm':
                ...
                condition = None  # Let it be random (will use generate_weather_condition)

            base_temp = -8 if 'winter' in scenario else 25 if 'summer' in scenario else None
            item = self.generate_forecast_item(item_timestamp, season, condition, base_temp)
...
if __name__ == "__main__":
    ...
    print(f"First item: {sample_data['list'][0]['dt_txt']} - {sample_data['list'][0]['weather'][0]['description']}")
